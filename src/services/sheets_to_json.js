export default async function fetchData(id, num, keepHeader = false) {
  const response = await fetch(
    `https://spreadsheets.google.com/feeds/cells/${id}/${num}/public/full?alt=json`
  );
  ...
  data["feed"]["entry"].forEach(cell => {
    ...
    let content = isNaN(Number(cell["content"]["$t"]))
      ? cell["content"]["$t"].trim()
      : Number(cell["content"]["$t"]);
    // Get Headers
    if (rownum == 0) {
